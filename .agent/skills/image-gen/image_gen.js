...
// 環境変数を読み込み (.env)
require('dotenv').config(); // カレントディレクトリの .env を読み込み

// ~/.claude/.env からも読み込み（カレントディレクトリの設定が優先）
...
if (!API_KEY) {
  console.error('[Image Gen] Error: GEMINI_API_KEY is not set in .env');
  process.exit(1);
}
...
async function generateImage(prompt, output, filename, referenceImages = []) {
  ...
  try {
    ...
    if (error.message.includes('API key')) {
      console.error(`\nTroubleshooting:`);
      console.error(`1. ~/.claude/.env に GEMINI_API_KEY が正しく設定されているか確認`);
...
function showHelp() {
  ...
  console.log('Environment Variables (~/.claude/.env):');
...
・見やすいスッキリした図解
・画像内に含まれるテキストは、明示的な言語指定がない限り、すべて日本語を使用すること
